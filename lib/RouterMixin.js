...
RouterMixin = {
    ...
    componentDidMount: function() {
        ...
        if (self.props.checkRouteOnPageLoad) {
            ...
            urlFromState = self.props && self.props.route && self.props.route.url;
        ...
        self._historyListener = function (e) {
            if (context) {
                ...
                debug('history listener invoked', e, url, self.props.route.url);
                if (url !== self.props.route.url) {
                    context.executeAction(navigateAction, {type: TYPE_POPSTATE, url: url, params: (e.props && e.props.params)});
                }
            }
        };
    ...
    componentDidUpdate: function (prevProps, prevState) {
        debug('component did update', prevProps, this.props);

        var newState = this.props;
        if (routesEqual(prevProps && prevProps.route, newState && newState.route)) {
            return;
        }
